...
const ADMIN_USERNAME = 'maxwill';
const ADMIN_EMAIL = 'iem@umpsa.edu.my';
...
async function adminLogin(username, password) {
  if (username.trim().toLowerCase() !== ADMIN_USERNAME) throw new Error('Incorrect username or password.');
...
function updateGalleryEditorCount() {
  const count = galleryEditorCards().length;
  if ($('galleryEditorCount')) $('galleryEditorCount').textContent = count;
  if ($('removeGalleryPhotoBtn')) $('removeGalleryPhotoBtn').disabled = count <= 6;
}
function addGalleryEditorCard(index, caption = '', value = null) {
...
function renderGalleryEditor() {
  ...
  for (let i = 0; i < getGalleryCount(); i++) addGalleryEditorCard(i, siteConfig.galleryCaptions[i] || '', mediaCache[`gallery-${i}`]);
...
async function saveForm() {
  ...
  try {
    ...
    const galleryCaptions = galleryCards.map((card, i) => card.querySelector(`#cfgGalleryCaption${i}`)?.value.trim() || DEFAULT_CONFIG.galleryCaptions[i] || `memory ${i+1} ♡`);
    siteConfig = {
      ...siteConfig,
    ...
    for (let i = 0; i < newGalleryCount; i++) {
      const file = $(`cfgGalleryPhoto${i}`)?.files?.[0];
      if (file) nextMedia[`gallery-${i}`] = await uploadAdminMedia(file, 'gallery', i);
    }
    Object.keys(nextMedia).forEach(key => {
      const match = key.match(/^gallery-(\d+)$/);
      if (match && Number(match[1]) >= newGalleryCount) delete nextMedia[key];
    });
